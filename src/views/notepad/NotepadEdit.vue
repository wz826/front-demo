...
import { Notebook, StarFilled, Plus, EditPen, Search, Key, } from '@element-plus/icons-vue';
...
onBeforeMount(async () => {
  ...
  notepad.tags.forEach(tag => tag.type = notepadTagType);
  // console.log(md.text);
});
...
const doSave = async () => {
  ...
  const ApifoxHost = "";
  // const ApifoxHost = import.meta.env.VENUS_WZBLOG_HOST;
  const SAVE_CD_API_URL = `${ApifoxHost}/notepad/editNote`;
...
<template>
  <div class="wzblog-notepad-edit-container">
    <breadcrumb />
    <div class="wzblog-notepad-edit-panel">
      ...
      <md-editor class="notepad-md-body" v-model="md.text" :editor-id="md.editorId" :theme="md.theme"
        :preview-theme="md.previewTheme" :preview="false" :code-theme="md.codeTheme"
        :show-code-row-number="md.showCodeRowNumber" :language="md.lang" :footers="md.footers" :toolbars="md.toolbars"
        :onUploadImg="onUploadImg" :onHtmlChanged="onHtmlChanged" :onSave="doSave" :onChange="onChange"
        :sanitize="toSanitizeHtml" :onGetCatalog="onGetCatalog" no-iconfont>
      ...
      <el-row :gutter="20" style="margin: 50px 120px;">
        <el-col :span="24">
          <el-form-item>
            <el-button type="primary" @click="doSave()" :icon="EditPen" size="large">更新笔记</el-button>
          </el-form-item>
        </el-col>
      </el-row>
    ...
    </div> <!-- end wzblog-notepad-edit-panel -->
  </div>
</template>

<style scoped>
.wzblog-notepad-edit-container {
  width: calc(100% - 6px);
  padding: 3px;
  display: flex;
