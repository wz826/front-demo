...
import { onBeforeMount, reactive, ref, Ref, toRaw } from 'vue';
...
const initEditData = async (canResetActivates: boolean = true) => {
    console.log('initEditData');
    /** 初始化 */
    const intros = await db.introduction.where('resumeId').equals(uuid).toArray();
    introConfig.intros.push(...intros);
    canResetActivates && introConfig.activates.push(intros.findIndex(it => it.isActivate));
    const dbskills = await db.skill.where('resumeId').equals(uuid).sortBy('sort');
    // console.log(dbskills);
    skills.value.push(...dbskills);
    const educations = await (await db.education.where('resumeId').equals(uuid).sortBy('startDate')).reverse();
    eduConfig.educations.push(...educations);
    canResetActivates && eduConfig.activates.push(0);
    const jobs = await (await db.job.where('resumeId').equals(uuid).sortBy('startDate')).reverse();
    jobConfig.jobs.push(...jobs);
    canResetActivates && jobConfig.activates.push(0);
    const projects = await (await db.project.where('resumeId').equals(uuid).sortBy('startDate')).reverse();
    projectConfig.projects.push(...projects);
    canResetActivates && projectConfig.activates.push(0);
    const dbresume = await db.resume.where('id').equals(uuid).last();
    Object.assign(resume, dbresume);
}
...
const removeIntroduction = (index: number) => {
    const del = introConfig.intros.splice(index, 1);
    if (introConfig.intros.length === 0) {
    ...
    // db.introduction.where('id').equals(del.shift()?.id as number).delete();
}
...
const removeSkill = (index: number) => {
    const del = skills.value.splice(index, 1);
    const id = del[0].id;
    console.log(id);
    // db.skill.where('id').equals(id as number).delete();
}
...
const removeEducation = (index: number) => {
    const del = eduConfig.educations.splice(index, 1);
    const acIdx = eduConfig.activates.findIndex(val => val === index);
    acIdx > -1 && eduConfig.activates.splice(acIdx, 1);
    // db.education.where('id').equals(del.shift()?.id as number).delete();
}
...
const saveResume = async () => {
    ...
    resume.utime = now;
    resume.isSync = false;
    introConfig.intros.forEach(it => it.utime = now);
    const rawSkills: Skill[] = skills.value.map((it, idx) => {
        it.utime = now;
        it.sort = idx;
        return toRaw(it);
    });
    eduConfig.educations.forEach(it => it.utime = now);
    jobConfig.jobs.forEach(it => it.utime = now);
    projectConfig.projects.forEach(it => it.utime = now);
    const tables = [db.resume, db.introduction, db.skill, db.education, db.job, db.project,];
    try {
        await db.transaction("rw", tables, async () => {
            let cnt = await db.resume.put(toRaw(resume));
            console.log(cnt);
            cnt = await db.introduction.bulkPut(toRaw(introConfig.intros));
            console.log(cnt);
            cnt = await db.skill.bulkPut(rawSkills);
            console.log(cnt);
            cnt = await db.education.bulkPut(toRaw(eduConfig.educations));
            console.log(cnt);
            cnt = await db.job.bulkPut(toRaw(jobConfig.jobs));
            console.log(cnt);
            cnt = await db.project.bulkPut(toRaw(projectConfig.projects));
            console.log(cnt);
        });
        loading.close();
        ...
        introConfig.intros.splice(0);
        skills.value.splice(0);
        eduConfig.educations.splice(0);
        jobConfig.jobs.splice(0);
        projectConfig.projects.splice(0);
        initEditData(false);
    } catch (e) {
        loading.close();
...
<template>
    <div class="wzblog-resume-edit-bg">
        <breadcrumb />
        <div class="wzblog-resume-edit-panel">
            <div class="wzblog-resume-edit-title">
                <div class="form-line">
                    <div class="form-line-label project-form-line-label">
                        <label :for="`resumeTitle`">简历标题</label>
                    </div>
                    <el-input v-model="resume.title" :name="`resumeTitle`" placeholder="简历标题"
                        :style="{ width: '500px' }" maxlength="50" clearable />
                </div>
                <el-button @click.stop="saveResume()" type="primary" :style="{ marginLeft: '30px' }">
                    <template #default>
                        <span>更新简历</span>
                    </template>
                </el-button>
                                ...
                                <el-collapse-item :name="idx">
                                        ...
                                        <div class="wzblog-resume-introduction-item-head">
                                            ...
                                            <div>
                                                <el-button @click.stop="activateIntroduction(idx)"
                                                    :type="intro.isActivate ? 'success' : 'primary'"
                                                    :disabled="intro.isActivate" :round="intro.isActivate">
                                                    <template #default>
                                                        <span v-text="intro.isActivate ? '使用中' : '使用'" />
                                                    </template>
                                                </el-button>
                                                <el-popconfirm title="你确定要删除这条记录吗?"
                                                    @confirm.stop="removeIntroduction(idx)">
                                                    <template #reference>
                                    ...
                                    <div class="wzblog-resume-introduction-item-body">
                                        ...
                                        <div class="form-line">
                                            ...
                                            <el-input v-model="intro.content" :name="`introContent${idx}`"
                                                placeholder="内容" type="textarea" :autosize="{ minRows: 5, maxRows: 13 }"
                                                :style="{ width: `800px` }" />
                                        </div>
                                    </div>
                                </el-collapse-item>
                            ...
                            <template #item="{ element: skill, index: idx }">
                                <div class="skill-form-line form-line" :key="skill.sort">
                                    <div class="form-line-label form-line-label-skill">
                                        <el-tooltip class="box-item" effect="dark" content="拖拽排序"
                                ...
                                <el-collapse-item :name="idx">
                                    <template #title>
                                        <div class="wzblog-resume-educations-item-head">
                                            ...
                                            <div>
                                                <el-popconfirm title="你确定要删除这条记录吗?"
                                                    @confirm.stop="removeEducation(idx)">
                                                    <template #reference>
                                                        <el-button type="danger" @click.stop="">
                                                            <template #default>
                                                                <span>删除</span>
                                    ...
                                    <div class="wzblog-resume-educations-item-body">
                                        <div class="form-line">
                                            ...
                                            <el-input v-model="edu.organization" :name="`eduOrg${idx}`"
                                                placeholder="教育机构" :style="{ width: '300px' }" maxlength="20"
                                                clearable />
                                            <div class="form-line-label edu-form-line-label">
                                            ...
                                            <div class="edu-monthrange">
                                                <el-date-picker v-model="edu.startEndDate"
                                                    ...
                                                    :name="[`eduStartEndDate${idx}`, `eduStartEndDate${idx}`]"
                                                    type="monthrange" start-placeholder="起始年月" range-separator="到"
                                                    end-placeholder="终止年月" unlink-panels :style="{ width: '280px' }"
                                                    :editable="false" popper-class="edu-monthrange-cell" />
                                            </div>
                                        </div>
                                        <div class="form-line">
                                            <div class="form-line-label edu-form-line-label">
                                            ...
                                            <el-input v-model="edu.certificateNo" :name="`eduCert${idx}`"
                                                placeholder="毕业证书编号" :style="{ width: '300px' }" maxlength="20"
                                                clearable />
                                        </div>
                                        <div class="form-line">
                                            <div class="form-line-label edu-form-line-label">
                                            ...
                                            <el-input v-model="edu.content" :name="`eduContent${idx}`"
                                                placeholder="教育内容" type="textarea"
                                                :autosize="{ minRows: 5, maxRows: 13 }" :style="{ width: `710px` }" />
                                        </div>
                                    </div>
                                </el-collapse-item>
                            </template>
                                    ...
                                    <div class="wzblog-resume-jobs-item-body">
                                        <div class="form-line">
                                            ...
                                            <div class="job-monthrange">
                                                <el-date-picker v-model="job.startEndDate"
                                                    :on-change="onChangeJobStartEndDate()"
                                                    :name="[`jobStartEndDate${idx}`, `jobStartEndDate${idx}`]"
                                                    type="monthrange" start-placeholder="入职年月" range-separator="到"
                                                    end-placeholder="离职年月" unlink-panels :style="{ width: '280px' }"
                                                    :editable="false" popper-class="job-monthrange-cell" />
                                            </div>
                                        </div>
                                        <div class="form-line">
                                            <div class="form-line-label job-form-line-label">
                                            ...
                                            <el-input v-model="job.department" :name="`jobDept${idx}`"
                                                placeholder="部门名称" :style="{ width: '300px' }" maxlength="20"
                                                clearable />
                                            <div class="form-line-label job-form-line-label">
                                                <label :for="`jobProf${idx}`">岗位名称</label>
                                            </div>
                                            <el-input v-model="job.profession" :name="`jobProf${idx}`"
                                                placeholder="岗位名称" :style="{ width: '300px' }" maxlength="20"
                                                clearable />
                                        </div>
                                        <div class="form-line">
                                            <div class="form-line-label job-form-line-label">
                                            ...
                                            <el-input v-model="job.content" :name="`jobContent${idx}`"
                                                placeholder="工作内容" type="textarea"
                                                :autosize="{ minRows: 5, maxRows: 13 }" :style="{ width: `810px` }" />
                                        </div>
                                        <div class="form-line">
                                            <div class="form-line-label job-form-line-label">
                                                <label :for="`jobAchi${idx}`">工作业绩</label>
                                            </div>
                                            <el-input v-model="job.achievement" :name="`jobAchi${idx}`"
                                                placeholder="工作业绩" type="textarea"
                                                :autosize="{ minRows: 5, maxRows: 13 }" :style="{ width: `810px` }" />
                                        </div>
                                    </div>
                                </el-collapse-item>
                                ...
                                <el-collapse-item :name="idx">
                                    ...
                                    <div class="wzblog-resume-projects-item-body">
                                        <div class="form-line">
                                            ...
                                            <el-input v-model="project.name" :name="`projectName${idx}`"
                                                placeholder="项目名称" :style="{ width: '400px' }" maxlength="20"
                                                clearable />
                                            <div class="form-line-label project-form-line-label">
                                        ...
                                        <div class="form-line">
                                            ...
                                            <el-input v-model="project.softwareEnv" :name="`projectSoft${idx}`"
                                                placeholder="软件环境" :style="{ width: '810px' }" maxlength="150"
                                                clearable />
                                        </div>
                                        <div class="form-line">
                                            <div class="form-line-label project-form-line-label">
                                            ...
                                            <el-input v-model="project.developTool" :name="`projectDevTool${idx}`"
                                                placeholder="开发工具" :style="{ width: '810px' }" maxlength="150"
                                                clearable />
                                        </div>
                                        <div class="form-line">
                                            <div class="form-line-label project-form-line-label">
                                            ...
                                            <el-input v-model="project.skill" :name="`projectSkill${idx}`"
                                                placeholder="技术点" :style="{ width: '810px' }" maxlength="150"
                                                clearable />
                                        </div>
                                        <div class="form-line">
                                            <div class="form-line-label project-form-line-label">
                                            ...
                                            <el-input v-model="project.desc" :name="`projectDesc${idx}`"
                                                placeholder="项目描述" type="textarea"
                                                :autosize="{ minRows: 5, maxRows: 13 }" :style="{ width: `810px` }" />
                                        </div>
                                        <div class="form-line">
                                            <div class="form-line-label project-form-line-label">
                                            ...
                                            <el-input v-model="project.duty" :name="`projectDuty${idx}`"
                                                placeholder="岗位职责" type="textarea"
                                                :autosize="{ minRows: 5, maxRows: 13 }" :style="{ width: `810px` }" />
                                        </div>
                                        <div class="form-line">
                                            <div class="form-line-label project-form-line-label">
                                            ...
                                            <el-input v-model="project.solution" :name="`projectSolution${idx}`"
                                                placeholder="技术解决方案" type="textarea"
                                                :autosize="{ minRows: 5, maxRows: 13 }" :style="{ width: `810px` }" />
                                        </div>
                                    </div>
                                </el-collapse-item>
...
@media (min-width: 1024px) {
    ...
    .wzblog-resume-edit-panel .ep-tabs {
        margin-top: 20px;
        width: 100%;
    }
