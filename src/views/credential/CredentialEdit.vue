...
const getEnable = async (id: any): Promise<any> => {
    // console.log(id.value);    
    // const ApifoxHost = import.meta.env.VENUS_ApifoxHost;
    const ApifoxHost = "";
    const API_URL = `${ApifoxHost}/credential/getEnable/${id}`;
...
watchEffect(async () => {
    ...
    const dbDataHead = await getEnable(id);
    // console.log(dbDataHead);
    const dbDataLine = dbDataHead.line;
    delete dbDataHead.line;
    Object.assign(formData, dbDataHead);
    Object.assign(formData.line, dbDataLine);
});
...
const submitForm = async (formEl: FormInstance | undefined) => {
    ...
    const SAVE_CD_API_URL = `${ApifoxHost}/credential/editCredential`;
...
<template >
    <div class="wzblog-credential-edit-container">
        <breadcrumb />
...
@media (min-width: 1024px) {
    ...
    .wzblog-credential-edit-container {
        width: calc(100% - 14px);
        padding: 3px;
    }
